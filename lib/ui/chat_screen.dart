...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void _sendMessage() async {
    try {
      ...
      user.save();
      await ConnectionService.sendMessage(id, user.id, text);
      WidgetsBinding.instance.addPostFrameCallback((_) => _scrollToBottom());
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<void> onAudioRecordingComplete(
    ...
    try {
      ...
      user.messages.add(msg);
      user.save();
      await ConnectionService.sendFile(id, widget.userId, filePath, fileName);
      WidgetsBinding.instance.addPostFrameCallback((_) => _scrollToBottom());
    } catch (e) {
      if (kDebugMode) {
  ...
  Widget build(BuildContext context) {
    return Consumer<ConnectionStateProvider>(
      builder: (context, connProvider, _) {
        ...
        Color statusColor;
        final isConnected =
            connProvider.connectedPeers.any((p) => p.userId == widget.userId);
        if (isConnected) {
                      ...
                      child: Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            IconButton(
                              onPressed: () => isConnected
                                  ? openCamera(_userBox, widget.userId,
